                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="rw" id="rw" required
                                ...
                                @if (Auth::user()->role->id === 1)
                                    <!-- Admin -->
                                    <option value="1" {{ isset($umkm) && $umkm->rw == '1' ? 'selected' : '' }}>1
                                    </option>
                                    <option value="2" {{ isset($umkm) && $umkm->rw == '2' ? 'selected' : '' }}>2
                                    </option>
                                    <option value="3" {{ isset($umkm) && $umkm->rw == '3' ? 'selected' : '' }}>3
                                    </option>
                                    <option value="4" {{ isset($umkm) && $umkm->rw == '4' ? 'selected' : '' }}>4
                                    </option>
                                    <option value="5" {{ isset($umkm) && $umkm->rw == '5' ? 'selected' : '' }}>5
                                    </option>
                                    <option value="6" {{ isset($umkm) && $umkm->rw == '6' ? 'selected' : '' }}>6
                                    </option>
                                    <option value="7" {{ isset($umkm) && $umkm->rw == '7' ? 'selected' : '' }}>7
                                    </option>
                                @else
                                    <!-- RW -->
                                    <option value="{{ Auth::user()->rw->id }}"
                                        {{ isset($umkm) && $umkm->rw == Auth::user()->rw->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="jumlah_umkm" id="jumlah_umkm"
                                placeholder="Silakan masukkan jumlah UMKM" value="{{ $umkm->jumlah_umkm ?? '' }}" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-500" />
                        </div>
                        <div>
                            ...
                            <select name="jenis_umkm" id="jenis_umkm" required
                                ...
                                <option value="Industri Pengolahan"
                                    {{ isset($umkm) && $umkm->jenis_umkm == 'Industri Pengolahan' ? 'selected' : '' }}>
                                    Industri Pengolahan</option>
                                <option value="Perdagangan besar/eceran"
                                    {{ isset($umkm) && $umkm->jenis_umkm == 'Perdagangan besar/eceran' ? 'selected' : '' }}>
                                    Perdagangan besar/eceran</option>
                                <option value="Penyedia Akomodasi & Makan Minum"
                                    {{ isset($umkm) && $umkm->jenis_umkm == 'Penyedia Akomodasi & Makan Minum' ? 'selected' : '' }}>
                                    Penyedia Akomodasi & Makan Minum</option>
                                <option value="Konstruksi"
                                    {{ isset($umkm) && $umkm->jenis_umkm == 'Konstruksi' ? 'selected' : '' }}>Konstruksi
                                </option>
                                <option value="Jasa Lainnya"
                                    {{ isset($umkm) && $umkm->jenis_umkm == 'Jasa Lainnya' ? 'selected' : '' }}>Jasa
                                    Lainnya</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="kategori_umkm" id="kategori_umkm" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-500">
                                <option value="Mikro"
                                    {{ isset($umkm) && $umkm->kategori_umkm == 'Mikro' ? 'selected' : '' }}>Mikro
                                </option>
                                <option value="Kecil"
                                    {{ isset($umkm) && $umkm->kategori_umkm == 'Kecil' ? 'selected' : '' }}>Kecil
                                </option>
                                <option value="Menengah"
                                    {{ isset($umkm) && $umkm->kategori_umkm == 'Menengah' ? 'selected' : '' }}>Menengah
                                </option>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="text" name="nama_pemilik" id="nama_pemilik"
                                placeholder="Silakan masukkan nama pemilik" value="{{ $umkm->nama_pemilik ?? '' }}"
                                required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-green-500" />
                        </div>
                        <div>
