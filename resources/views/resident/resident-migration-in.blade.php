...
@section('dashboard-title')
    Sista Bijak - Tabel Penduduk Masuk
@endsection
...
@section('body')
    ...
    <div class="container mx-auto px-4 py-6">
        <div class="card shadow-lg rounded-lg overflow-hidden">
            <div class="bg-gray-800 text-white p-4 flex justify-between items-center">
                <h3 class="text-lg font-bold">Tabel Penduduk Masuk</h3>
            ...
            <div class="p-4">
                            ...
                            @else
                                @foreach ($penduduk as $index => $data)
                                    <tr class="hover:bg-gray-100 transition duration-200">
                                        <td class="px-4 py-2 whitespace-nowrap text-center">{{ $penduduk->firstItem() + $index }}.</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->nik }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap uppercase">{{ $data->nama_lengkap }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->jenis_kelamin }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->tempat_lahir }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->tanggal_lahir }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->status_hubkel }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->pendidikan_terakhir }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->jenis_pekerjaan }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->agama }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->status_perkawinan }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap uppercase">
                                            <button class="bg-blue-500 text-white px-4 py-2 rounded ml-2 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition ease-in-out duration-150"
                                                onclick="showAddressModal('{{ $data->alamat }}')">Lihat</button>
                                        </td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->rw }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->rt }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->kelurahan }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center uppercase">{{ $data->status_kependudukan }}</td>
                                        <td class="px-4 py-2 whitespace-nowrap text-center space-x-2">
                                            <a href="{{ route('migrasimasuk.edit', $data->nik) }}" title="Edit data"
                                            ...
                                            <form action="{{ route('migrasimasuk.destroy', $data->id) }}" method="POST" class="inline" id="deleteForm">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="mt-4">
                    {{ $penduduk->appends(request()->except('page'))->links() }} <!-- Pagination links -->
                </div>
            </div>
        </div>
        ...
        <div id="dataEntryModal" class="fixed inset-0 z-10 flex items-center justify-center bg-gray-800 bg-opacity-60 hidden">
            <div id="dataEntryContent" class="bg-white p-6 rounded-lg shadow-lg relative w-full max-w-xl fadeIn">
                ...
                <form id="dataForm">
                    <input type="number" id="dataAmount" min="1" class="border border-gray-300 rounded-md p-2 w-full mb-4" placeholder="Masukkan jumlah data">
    ...
    <div class="card shadow-lg rounded-lg overflow-hidden mb-4">
        <div class="bg-gray-800 text-white p-4">
            <h3 class="text-lg font-bold">Data Penduduk ${index + 1}</h3>
        </div>
        <div class="p-4">
    ...
    <!-- Modal Show -->
    <div id="addressModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black bg-opacity-50 transition-opacity">
        <div id="modalContent" class="bg-white rounded-lg shadow-lg w-11/12 max-w-lg max-h-[80vh] overflow-y-auto modal-enter">
            ...
            <div class="flex justify-end p-4 border-t">
                <button onclick="closeAddressModal()" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Tutup</button>
            </div>
        </div>
    </div>
@endsection